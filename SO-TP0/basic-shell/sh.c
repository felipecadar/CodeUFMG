...
/* MARK NAME Felipe Cadar Chamone */
...
runcmd(struct cmd *cmd)
{
  ...
  case ' ':
     ...
     * comandos simples. */
    // execvp -> fins "ls" on $PATH and execute
    execvp(ecmd->argv[0], ecmd->argv);
    // fprintf(stderr, "exec ls nao implementado\n");
    /* MARK END task2 */
    break;
  ...
  case '<':
    ...
    // fprintf(stderr, "Opened %d close %d \n", fd, rcmd->fd);
    if (r < 0){
      fprintf(stderr, "Fail open file %s with mode %d\n", rcmd->file, rcmd->mode);
      exit(0);
    }
    dup2(r, rcmd->fd);
  ...
  case '|':
    ...
    if(left == 0){ //im child
      ...
      dup(p[1]); // duplicate stdout to new fd
    ...
    if(right == 0){ //im child
      close(0); // close stdin
      dup(p[0]); // duplicate stdin to new fd;
      close(p[0]); // close original fd
      close(p[1]); // close original fd
      runcmd(pcmd->right);
    }
    close(p[0]); // close father fd
    close(p[1]); // close father fd

    wait(NULL); // wait both process end
    wait(NULL);
    /* MARK END task4 */
    break;
  }    
