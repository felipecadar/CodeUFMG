...
dir_path = "log/sensitivity_analysis/cycle/datasets/graph2/"
...
parms_file = dir_path.replace("log/", "")+"/exp_3.pk"
...
for k in range(len(exps)):
    ...
    for i in range(rep):
        ...
        for j in range(len(lines)):
            ...
            general_max[j][k] = np.max(x)
            general_min[j][k] = np.min(x)
            general_mean[j][k] = np.mean(x)

    

    final_max = np.mean(general_max, 1)
    final_min = np.mean(general_min, 1)
